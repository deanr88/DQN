...
class GoddardEnv(gym.Env):
    ...
    def __init__(self, rocket=Default()):
        ...
        self.observation_space = gym.spaces.Box(
            low   = np.array([-0.5, self._r.H0, self._r.M1]),
            high  = np.array([1.5, self._r.H_MAX_RENDER, self._r.M0]),
            dtype = np.float
        )
    ...
    def step(self, action):
        ...
        self._h_max = self._state[self.H_INDEX] #max(self._h_max, self._state[self.H_INDEX]) #self._state[self.H_INDEX]

        is_done = bool(
            is_tank_empty and self._state[self.V_INDEX] < 0 and self._h_max > 1.010
        )


        if is_done:
            reward = (self._h_max - self._r.H0)*500
        else:
            reward = -1.0
